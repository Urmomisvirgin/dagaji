...
def process_video_transcript(url, model):
    ...
    for chunk in chunks:
        input_token_count += helpers.count_tokens(chunk, model)
        prompt = youtube.best_youtube_summary.format(user_input=chunk)
        try:
            chunk_response, response_data = llm.llm_generate_text_with_save(prompt, "OpenAI", model)
            overall_output += chunk_response
            if response_data:  # Ensure response_data is not None
                output_token_count += response_data['completion_tokens']
